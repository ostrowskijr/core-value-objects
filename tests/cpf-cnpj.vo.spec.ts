...
import { InvalidCpfCnpjException, InvalidCpfException, InvalidCnpjException, InvalidDocumentLengthException } from '../src/exceptions/cpf.exceptions';
...
describe('CpfCnpj', () => {
  ...
  it('Deve lançar erro devido primeiro digito inválido', () => {
    expect(() => new CpfCnpj('123.456.789-99')).toThrow(InvalidCpfException);
  });
  it('Deve lançar erro devido segundo digito inválido', () => {
    expect(() => new CpfCnpj('123.456.789-00')).toThrow(InvalidCpfException);
  });
  ...
  it('deve lançar erro para CNPJ inválido', () => {
    expect(() => new CpfCnpj('00.000.000/0000-00')).toThrow(InvalidCnpjException);
  });
  ...
  it('deve retornar false para string com tamanho inválido', () => {
    // 12 dígitos
    expect(() => new CpfCnpj('123456789012')).toThrow(InvalidDocumentLengthException);
    // 13 dígitos
    expect(() => new CpfCnpj('1234567890123')).toThrow(InvalidDocumentLengthException);
    // string vazia
    expect(() => new CpfCnpj('')).toThrow(InvalidDocumentLengthException);
  });

  it('deve retornar false para CPF e CNPJ com todos os dígitos iguais', () => {
    expect(() => new CpfCnpj('111.111.111-11')).toThrow(InvalidCpfException);
    expect(() => new CpfCnpj('22222222222222')).toThrow(InvalidCnpjException);
  });
