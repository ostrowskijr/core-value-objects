...
describe('Nome', () => {
  ...
  describe('Validações de erro', () => {
    it('deve lançar erro para nome com apenas uma palavra', () => {
      expect(() => new Nome('João')).toThrow('Nome deve conter ao menos nome e sobrenome.');
    });

    it('deve lançar erro para nome vazio', () => {
      expect(() => new Nome('')).toThrow('Nome deve conter ao menos nome e sobrenome.');
      expect(() => new Nome('   ')).toThrow('Nome deve conter ao menos nome e sobrenome.');
    });

    it('deve lançar erro para nome com apenas uma letra', () => {
      expect(() => new Nome('J Silva')).toThrow('Nome deve conter ao menos nome e sobrenome.');
      expect(() => new Nome('João S')).toThrow('Nome deve conter ao menos nome e sobrenome.');
    });

    it('deve lançar erro para nome com caracteres inválidos', () => {
      expect(() => new Nome('João123 Silva')).toThrow('Nome deve conter ao menos nome e sobrenome.');
      expect(() => new Nome('João Silva!')).toThrow('Nome deve conter ao menos nome e sobrenome.');
    });

    it('deve lançar erro para nome com apenas espaços, hífens ou apóstrofos', () => {
      expect(() => new Nome('- Silva')).toThrow('Nome deve conter ao menos nome e sobrenome.');
      expect(() => new Nome("João '")).toThrow('Nome deve conter ao menos nome e sobrenome.');
    });

    it('deve lançar erro para nome undefined ou null', () => {
      expect(() => new Nome(undefined as any)).toThrow('Nome deve conter ao menos nome e sobrenome.');
      expect(() => new Nome(null as any)).toThrow('Nome deve conter ao menos nome e sobrenome.');
    });
  });
