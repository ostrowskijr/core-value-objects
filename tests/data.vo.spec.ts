...
import { InvalidDataException } from "../src/exceptions/data.exceptions";
...
describe("Data", () => {
  ...
  it("deve lançar erro para data inválida em formato string", () => {
    expect(() => new Data("data invalida")).toThrow(InvalidDataException);
    expect(() => new Data("2024-02-31")).toThrow(InvalidDataException);
  });

  it("deve lidar corretamente com anos bissextos", () => {
    const data = new Data("2024-02-29");
    expect(data.getValueFormatted()).toBe("29/02/2024");
    expect(() => new Data("2023-02-29")).toThrow(InvalidDataException);
  });
