import { InvalidCRPException } from "../exceptions/crp.exceptions";
...
export class Crp implements IValueObjects<string> {
  ...
  constructor(value: string) {
    ...
    if (!Crp.validate(valueClean)) throw new InvalidCRPException();
  ...
  getValueFormatted = (): string =>
    this.value.replace(/(\d{2})(\d{5})/, "$1/$2");
  equals = (value: IValueObjects): boolean =>
    value instanceof Crp && this.value === value.getValue();
}
